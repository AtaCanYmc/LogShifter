...
async def test_sheets_adapter_dry_run():
    adapter = SheetsAdapter(
        service_account_file="creds.json", spreadsheet_id="sheet_123", worksheet_name="Logs"
    )
    res = await adapter.ship(logs=[{"id": 1, "message": "hello"}], target="sheet_123", dry_run=True)
    assert res is True
...
async def test_sheets_adapter_real_flow(mock_sa):
    ...
    res = await adapter.ship(
        logs=[{"id": 1, "message": "hello"}], target="sheet_123", dry_run=False
    )
    assert res is True
